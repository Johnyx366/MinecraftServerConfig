...
The app asks for confirmation before console commands that can alter server/player state, including `stop`, operator/ban/whitelist commands, difficulty or gamerule changes, worldborder/forceload operations, saving controls, and reload. Normal commands such as `say` run directly. Its backup card shows the most recent snapshot, its age and processed world size; **Backup now** performs the normal consistent restic backup and integrity check. The diagnostics row shows the Java process CPU/RAM while running, free disk, and opens the full console log.
